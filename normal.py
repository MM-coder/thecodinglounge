    ...
    async def ban(self, ctx, member: discord.Member):
        if ctx.message.author.server_permissions.administrator or ctx.message.author.id == '397745647723216898':
            try:
                ...
                await self.bot.say(":thumbsup: Succesfully issued a ban!")
    ...
    async def kick(self, ctx, member: discord.Member):
        if ctx.message.author.server_permissions.administrator or ctx.message.author.id == '397745647723216898':
            try:
                ...
                await self.bot.say("Succesfully kicked that member!")
            except discord.errors.Forbidden:
                await self.bot.say(":x: No permisson!")
        else:
            await self.bot.say("You dont have perms")
